	...
	var interesting_subdomain_table = $('#interesting_subdomains').DataTable({
		"drawCallback": function(settings, start, end, max, total, pre) {
			...
			if (this.fnSettings().fnRecordsTotal() == 0) {
				...
				// <div class="card-body">
				// asm-Hawk could not identify any interesting subdomains. You can customize interesting subdomain keywords <a href="/scanEngine/interesting/lookup/">from here</a> and this section would be automatically updated.
				// </div>
				// </div>`);
			} else {
				// show nav bar
	...
	fetch('/api/fetch/most_vulnerable/?format=json', {
		...
		if (response.status) {
			...
			$('#most_vulnerable_target_div').append(`
				...
				Once the vulnerability scan is performed, asm-Hawk will identify the most vulnerable targets.</div>
			`);
		}
	});
}
	...
	fetch('/api/fetch/most_common_vulnerability/?format=json', {
		...
		if (response.status) {
			...
			$('#most_common_vuln_div').append(`
				...
				Once the vulnerability scan is performed, asm-Hawk will identify the Most Common Vulnerabilities.</div>
			`);
		}
	});
}
